...
Reuses `vrm_live.py`'s codes/discovery discipline (same multi-instance PV
trap, same "no data is better than fabricated data" rule) rather than
restating them — this module answers "what shape," that one answers "what
right now," over the same underlying signals.
"""
import logging
...
def fetch_site_shape(client, id_site, *, range_key: str, tz: str = DEFAULT_TZ_NAME) -> dict:
    ...
    solar_series = series_by_code.get(PV_POWER_CODE, pd.Series(dtype=float))
