...
| 23 — Cotizaciones: single-margin profit distribution — a Cotización never shows an explicit "Utilidad" line; Oscar sets one overall margin % per quote (app-suggested from historical project data) and every cost line's shown price becomes `costo_real × (1+margen)`, except the flat Permiso de Interconexión government fee, kept pass-through at cost. Migration `049` adds a `markup_eligible` flag to `service_defaults` (plus an admin toggle for it) and folds "Estructura de montaje" into the shared catalog for all three system types instead of Off-Grid/Hybrid only. **Supersedes** `calculations/profit-distribution-quotation-system.md`'s original fixed-anchor/residual design — Oscar asked for this simpler mechanism instead after reviewing that plan; see the plan's own "Revision note." | ✅ All 4 steps complete and verified 2026-09-26, on the `main_jinja` branch/worktree — `main` (Streamlit) is untouched; this feature exists only in the Flask port. See `PLAN_PHASE23_PROFIT_DISTRIBUTION.md` and `CONTEXT.md`'s own section for the full design and a real bug found+fixed mid-build (`database/equipment_db.py:list_service_defaults()` wasn't selecting the new `markup_eligible` column, so every row silently defaulted to markup-eligible). The margin-suggestion helper has no history to suggest from yet — this Supabase project currently has 0 rows in `projects` — and degrades to a blank/0% default until the first quote is promoted. |
